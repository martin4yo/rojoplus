...
router.get('/', authAdmin, asyncHandler(async (req, res) => {
  ...
  const [gestiones, total] = await Promise.all([
    req.db.gestionCobranza.findMany({
      where,
      skip,
      take: parseInt(limit),
      include: {
    ...
    req.db.gestionCobranza.count({ where })
  ])
...
router.get('/estadisticas', authAdmin, asyncHandler(async (req, res) => {
  const [
    ...
    req.db.gestionCobranza.count(),
    req.db.gestionCobranza.groupBy({
      by: ['prioridad'],
      _count: true
    }),
    req.db.gestionCobranza.groupBy({
    ...
    req.db.gestionCobranza.aggregate({
...
router.get('/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  const gestion = await req.db.gestionCobranza.findUnique({
...
router.post('/:id/acciones', authAdmin, asyncHandler(async (req, res) => {
  ...
  // Crear la acción
  const accion = await req.db.accionCobranza.create({
    data: {
  ...
  await req.db.gestionCobranza.update({
    where: { id: parseInt(id) },
    data: {
      ultimaAccion: tipo,
...
router.put('/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  const updated = await req.db.gestionCobranza.update({
    where: { id: parseInt(id) },
    data: {
      estado,
...
router.delete('/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  await req.db.gestionCobranza.delete({
    where: { id: parseInt(id) }
  })
