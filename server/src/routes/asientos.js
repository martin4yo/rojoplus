...
router.get('/resumen/balance', authAdmin, asyncHandler(async (req, res) => {
  ...
  const cuentas = await req.prisma.cuentaContable.findMany({
    where: { activo: true },
    include: {
      asientoLineas: {
        where: {
...
router.get('/libro-mayor/:cuentaId', authAdmin, asyncHandler(async (req, res) => {
  ...
  const cuenta = await req.prisma.cuentaContable.findUnique({
  ...
  const [lineas, total] = await Promise.all([
    req.prisma.asientoLinea.findMany({
      where: {
  ...
  // Totales
  const totales = await req.prisma.asientoLinea.aggregate({
    where: {
      cuentaContableId: parseInt(cuentaId),
      asiento: whereAsiento
    },
...
router.get('/', authAdmin, asyncHandler(async (req, res) => {
  ...
  const [asientos, total] = await Promise.all([
    ...
    req.prisma.asiento.count({ where })
  ])
...
router.get('/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  const asiento = await req.prisma.asiento.findUnique({
    where: { id: asientoId },
    include: {
      lineas: {
        include: {
...
router.post('/', authAdmin, asyncHandler(async (req, res) => {
  ...
  const asiento = await req.prisma.asiento.create({
    data: {
      numero,
...
router.post('/:id/anular', authAdmin, asyncHandler(async (req, res) => {
  ...
  const asientoAnulado = await req.prisma.asiento.update({
    where: { id: parseInt(id) },
    data: {
      estado: 'ANULADO',
...
router.delete('/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  await req.prisma.asiento.delete({
    where: { id: parseInt(id) }
  })
