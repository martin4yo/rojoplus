...
router.put('/actividades/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  const actividad = await req.db.actividad.update({
    where: { id: parseInt(id) },
    data: {
...
router.delete('/actividades/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  await req.db.actividad.delete({ where: { id: parseInt(id) } })
...
router.delete('/categorias-actividad/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  const inscripcionesCount = await req.db.inscripcion.count({
...
// Recibe el cliente (tx tenant-scoped o req.db) como parámetro.
async function generarCodigoEntidadPersonal(db) {
  const prefijo = 'PERS-'
  const ultima = await db.entidad.findFirst({
