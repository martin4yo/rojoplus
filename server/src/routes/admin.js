...
router.get('/dashboard', authAdmin, asyncHandler(async (req, res) => {
  ...
  try {
    tarjetasPendientes = await req.prisma.movimientoCaja.findMany({
      where: {
        conciliado: false,
        anulado: false,
        tipo: 'INGRESO',
  ...
  const movimientosHistoricos = await req.prisma.movimientoCaja.findMany({
...
router.delete('/medios-pago/:id', authAdmin, asyncHandler(async (req, res) => {
  ...
  const pagosConMedio = await req.prisma.pago.count({
...
router.put('/configuracion/:clave', authAdmin, asyncHandler(async (req, res) => {
  ...
  const config = await req.prisma.configuracion.upsert({
    where: { clave },
    update: { valor },
    create: {
      clave,
