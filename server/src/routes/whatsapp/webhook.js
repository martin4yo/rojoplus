...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    const agentConfig = await db.configuracion.findUnique({
      where: { tenantId_clave: { tenantId, clave: 'WA_AGENT_ENABLED' } }
    })
    ...
    const whitelistConfig = await db.configuracion.findUnique({
      where: { tenantId_clave: { tenantId, clave: 'WA_AGENT_WHITELIST' } }
    })
    ...
    const horaInicioConfig = await db.configuracion.findUnique({
      where: { tenantId_clave: { tenantId, clave: 'WA_AGENT_HORARIO_INICIO' } }
    })
    const horaFinConfig = await db.configuracion.findUnique({
      where: { tenantId_clave: { tenantId, clave: 'WA_AGENT_HORARIO_FIN' } }
    })
    ...
    if (horaActual < horaInicio || horaActual >= horaFin) {
      ...
      const msgFueraHorario = await db.configuracion.findUnique({
        where: { tenantId_clave: { tenantId, clave: 'WA_AGENT_MSG_FUERA_HORARIO' } }
      })
