...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const menuItems = await req.db.menuItem.findMany({
    where: {
      activo: true,
      parentId: null
    },
...
router.post('/', checkPermiso('USUARIOS_GESTIONAR'), asyncHandler(async (req, res) => {
  ...
  if (parentId) {
    const parent = await req.db.menuItem.findUnique({
  ...
  const menuItem = await req.db.menuItem.create({
    data: {
...
router.put('/:id', checkPermiso('USUARIOS_GESTIONAR'), asyncHandler(async (req, res) => {
  ...
  const existing = await req.db.menuItem.findUnique({
  ...
  const menuItem = await req.db.menuItem.update({
    where: { id: parseInt(id) },
    data: {
...
router.patch('/:id/roles', checkPermiso('USUARIOS_GESTIONAR'), asyncHandler(async (req, res) => {
  ...
  const updated = await req.db.menuItem.findUnique({
    where: { id: parseInt(id) },
    include: {
      roles: {
        include: { rol: { select: { id: true, codigo: true, nombre: true } } }
...
router.post('/seed', checkPermiso('USUARIOS_GESTIONAR'), asyncHandler(async (req, res) => {
  ...
  for (const item of menuData) {
    ...
    if (!parent) {
      // Crear el padre
      parent = await req.db.menuItem.create({
        data: {
          titulo: item.titulo,
          icono: item.icono,
          url: item.url || null,
      ...
      const realParent = await req.db.menuItem.findFirst({
    ...
    if (item.children && parent.id) {
      for (const child of item.children) {
        const childExists = await req.db.menuItem.findFirst({
          where: { url: child.url }
        })

        if (!childExists) {
          await req.db.menuItem.create({
            data: {
              parentId: parent.id,
              titulo: child.titulo,
              icono: child.icono,
  ...
  const total = await req.db.menuItem.count()
